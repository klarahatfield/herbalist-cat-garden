...
function buildLocScroll(){
  const w = document.getElementById('loc-scroll');
  w.innerHTML = '';
  w.style.cssText = 'position:relative;width:100%;padding-bottom:0';

  // 지도 이미지
  const img = document.createElement('img');
  img.src = 'image/map2.png'; // 실제 경로로 변경
  img.style.cssText = 'width:100%;display:block;border-radius:12px';
  w.appendChild(img);

  // 구역별 클릭 영역 (좌% 상% 너비% 높이% — 지도 비율에 맞게 조정)
  const zones = [
    { key:'bloom',   left:'5%',  top:'55%', w:'25%', h:'20%' },
    { key:'pond',    left:'22%', top:'35%', w:'28%', h:'25%' },
    { key:'forest',  left:'0%',  top:'25%', w:'20%', h:'30%' },
    { key:'hills',   left:'25%', top:'5%',  w:'30%', h:'20%' },
    { key:'whisker', left:'53%', top:'30%', w:'22%', h:'20%' },
    { key:'lookout', left:'70%', top:'8%',  w:'28%', h:'22%' },
    { key:'mushroom',left:'75%', top:'45%', w:'22%', h:'25%' },
    { key:'cottage', left:'48%', top:'55%', w:'22%', h:'28%' },
  ];

  const shortNames = {
    bloom:'몽환꽃밭', pond:'달빛라군', forest:'가르릉숲',
    hills:'벨벳이끼언덕', mushroom:'버섯밭', whisker:'위스커정원',
    lookout:'스크래치전망대', cottage:'오두막'
  };

  zones.forEach(z => {
    const unlocked = isLocUnlocked(z.key);
    const unlockDay = getLocUnlockDay(z.key);

    const area = document.createElement('div');
    area.dataset.loc = z.key;
    area.style.cssText = `
      position:absolute; left:${z.left}; top:${z.top};
      width:${z.w}; height:${z.h};
      border-radius:8px; cursor:${unlocked ? 'pointer' : 'default'};
      transition:all .2s;
      ${z.key === G.curLoc ? 'box-shadow:0 0 0 3px rgba(255,200,80,.8);background:rgba(255,200,80,.15)' : ''}
    `;

    if(!unlocked){
      area.innerHTML = `<div style="position:absolute;bottom:2px;left:50%;transform:translateX(-50%);
        background:rgba(0,0,0,.6);color:#aaa;font-size:.6rem;padding:2px 5px;
        border-radius:4px;white-space:nowrap">🔒 Day ${unlockDay}</div>`;
      area.onclick = () => showResult(`🔒 Day ${unlockDay}에 해금됩니다!`, true);
    } else {
      area.innerHTML = `<div style="position:absolute;bottom:2px;left:50%;transform:translateX(-50%);
        background:rgba(0,0,0,.55);color:#fff;font-size:.62rem;padding:2px 6px;
        border-radius:4px;white-space:nowrap">${shortNames[z.key]||z.key}</div>`;
      area.onmouseenter = () => { if(z.key !== G.curLoc) area.style.background = 'rgba(255,200,80,.2)'; };
      area.onmouseleave = () => { if(z.key !== G.curLoc) area.style.background = ''; };
      area.onclick = () => setLocation(z.key);
    }
...
function setLocation(key){
  ...
  document.querySelectorAll('[data-loc]').forEach(a=>{
    const isActive=a.dataset.loc===key;
   a.style.boxShadow='';
    a.style.background='';
    });
  // 나머지 기존 코드 그대로...
  ...
  document.getElementById('sp-label').textContent=(G.isNight&&L.sNightName)?L.sNightName:L.sName;
  ...
  const ITEM_ICONS={herb:'🌿',lotus:'🪷',shroom:'🍄',moss:'🪨',resin:'🌳',rare:'💎', petal:'🌸'};
  const ITEM_LABELS={herb:'약초',lotus:'수초',shroom:'버섯',moss:'이끼',resin:'수지',rare:'희귀초', petal:'꽃잎'};
  ...
  if(isNightNow2 && L.nItem){
    ...
    if(gl&&L.item) gl.textContent=L.gatherLabel||(ITEM_LABELS[L.item]||'채집')+' 채집';
  }
...
// ══════════════════════════════════════
const RECIPES={
  petal_brew:{name:'🌸 꽃잎 물약', need:{petal:3,herb:1}, reward:{silver:1,bronze:3}},
  honey_syrup:{name:'🍯 꿀 시럽', need:{honey:2,petal:1}, reward:{bronze:3}},
  moon_honey:{name:'🌙 달빛 꿀차', need:{honey:2,lotus:1}, reward:{silver:1}},
  golden_honey:{name:'✨ 황금 꿀약', need:{honey:3,rare:1}, reward:{silver:2}},
