...
function updateTitle(){
  ...
  if(name){
    titleEl.textContent='🌿 '+name+'님을 환영합니다';
  } else {
    titleEl.textContent='🌿 환영합니다';
  }
...
const SPECIAL_ACTIONS = {
  bloom: {
    name:'꽃꿀 채취', icon:'🌸', cost:2,
    fn: function(){
      addHerb('lotus',2); addHerb('rare',1);
      showResult('🌸 꽃꿀 채취! 달빛 수초×2 + 희귀초×1');
  ...
  openResinMinigame(
    function(){
      addHerb('resin',3);
      showResult('🌳 수지 추출 성공! 수지×3 획득!');
      showBabaComment('"나무 상처에서 나오는 수지... 자연의 눈물이라 부르지."');
    },
    function(){ showResult('수지 추출 실패...'); }
  );}, 
},
  hills: {
