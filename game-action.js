...
function selectChar(type){
  ...
  if(prev) prev.textContent = '"약초사 ' + nm + '님의 비밀정원"';
}
  ...
  if(s.type==='locpick'){
    ...
    PICK_LOCS.forEach(function(lc){
      ...
      b.onclick=function(){
        ...
        var locTip={bloom:'꽃잎과 약초가 많아. 연금술 재료로 딱이지.',pond:'연꽃못엔 달빛이 고여. 수초와 이끼를 찾아봐.',hills:'이끼언덕은 조용한 곳이야. 수지도 나오거든.',mushroom:'버섯 군락은 독특한 재료 보고야. 밤엔 더 진귀한 게 자라지.'}[lc.key]||'좋은 선택이야.';
...
const HOUSE_MODS={
  ...
  roof:{name:'전망대',icon:'🔭',desc:'탐색 범위.',mods:[
...
function renderHouseTab(){
  ...
  if(!G.houseMods)G.houseMods={};
  sec.innerHTML='';
  var parts=Object.keys(HOUSE_MODS);
  for(var pi=0;pi<parts.length;pi++){
    ...
    wrap.appendChild(ph);
    for(var mi=0;mi<part.mods.length;mi++){
      var mod=part.mods[mi];
      var isOwned=mod.owned||(G.houseMods[mod.id]==='owned'||G.houseMods[mod.id]==='equipped');
      var isEquip=false;
      if(G.houseMods[mod.id]==='equipped'){
        isEquip=true;
      } else if(mod.price===0){
      ...
      // 액션 버튼 - createElement로 따옴표 충돌 완전 회피
      var actionEl=document.createElement('div');
      if(isEquip){
        actionEl.innerHTML='<span class="hmc-owned">장착중</span>';
      } else if(isOwned){
        var btn=document.createElement('button');
        btn.style.cssText='padding:4px 10px;border-radius:6px;border:1px solid var(--moss);font-size:.72rem;cursor:pointer';
        btn.textContent='장착';
        (function(pid,mid){btn.onclick=function(e){e.stopPropagation();equipMod(pid,mid);};})(partId,mod.id);
