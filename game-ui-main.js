...
// ══════════════════════════════════════
const SH_PRICES={energy:[2,3,5,8],gather:[1,2,3],luck:[2,3,5]};
...
//  HELPERS
// ══════════════════════════════════════
const IN={herb:'약초',lotus:'수초',shroom:'버섯',moss:'이끼',resin:'수지',rare:'희귀초',moonpetal:'달빛꽃잎',starweed:'별빛풀',glowshroom:'발광버섯',nightmoss:'야광이끼',shadowleaf:'그림자잎',lumiresin:'빛수지',snowcrystal:'눈결정초',sakuradew:'벚꽃이슬',sunbloom:'태양꽃',crimsonleaf:'단풍잎',spring_essence:'봄의 정수',summer_essence:'여름의 정수',autumn_essence:'가을의 정수',winter_essence:'겨울의 정수'};
...
function updateUI(){
  ...
  const can={
    ...
    legendary:h.rare>=1&&h.herb>=1&&h.lotus>=1&&h.shroom>=1&&h.moss>=1&&h.resin>=1,
  };
  ...
  // 물약 인벤토리 표시
  if(G.potionInv){
    const pkeys=['healing','moon','forest','dream','legendary'];
...
const HERBARIUM = [
  {
    id:'herb', icon:'🌿', reqItem:'herb', reqAmt:10,
    name:'털뭉치 엉겅퀴', engName:'Fuzz-Thistle',
    latin:'Carduus fluffy-wuffy',
    habitat:'폐허가 된 공장단지 담벼락 아래',
    effect:'뿌리에서 추출한 즙은 타박상 부위의 붓기를 빠르게 가라앉힌다.',
    note:'이 엉겅퀴를 꺾을 때 나는 소리가 고양이의 가르랑 소리와 99% 일치한다. 그래서인지 이걸 약으로 쓰려고 짓이길 때마다 자꾸 죄책감이 들어서, 짓이기기 전에 꼭 5분 동안 쓰다듬어줘야 한다. 안 그러면 잎이 혓바닥처럼 말려 올라와 내 손가락을 핥는다. 찝찝하다.',
    gameEffect:'🎲 채집 시 5% 확률로 Activity +1 회복',
  },
  {
    id:'lotus', icon:'🪷', reqItem:'lotus', reqAmt:8,
    name:'달빛 연꽃', engName:'Lunar Lure Lily',
    latin:'Nymphaea lunaris',
    habitat:'루나루어 라군 수면 위, 달빛이 닿는 곳',
    effect:'꽃잎을 달인 물은 수면 장애와 불안을 완화하며, 달이 밝을수록 효능이 강해진다.',
    note:'이 꽃은 내가 말을 걸면 꽃잎을 오므린다. 처음엔 수줍은 줄 알았다. 알고 보니 내 목소리 주파수가 꽃의 불안 주파수와 일치한다고 한다. 어쩐지 나랑 있으면 다들 긴장하더라.',
    gameEffect:'🌙 달빛 물약 제조 시 효율 +1',
  },
  {
    id:'shroom', icon:'🍄', reqItem:'shroom', reqAmt:10,
    name:'태엽 감는 버섯', engName:'Winder-Shroom',
    latin:'Fungus mechanicus',
    habitat:'버려진 공장의 천장, 혹은 태엽 장치 옆',
    effect:'가루를 내어 마시면 무거운 물건을 들어 올릴 수 있는 일시적인 근력 강화.',
    note:'갓 부분을 살짝 돌려주면 "태엽 감기는 소리"가 나면서 버섯이 팽팽하게 굳는다. 이걸 샐러드에 넣어 먹으면 힘이 넘쳐나서 의도치 않게 실험실 문짝을 뜯어버리곤 한다. 문제는 태엽이 풀릴 때가 되면 갑자기 기운이 다 빠져서 그 자리에서 털썩 쓰러져 잠들게 된다는 점이다. 아까도 20kg짜리 연금술 가마솥을 든 채로 잠들어서 3시간 동안 꼼짝 못 했다.',
    gameEffect:'⚡ 버섯 5개 보유 시 하루 Activity +1',
  },
  {
    id:'moss', icon:'🪨', reqItem:'moss', reqAmt:8,
    name:'증기 뿜는 이끼', engName:'Steam-Vent Moss',
    latin:'Muscus vaporis',
    habitat:'지하 실험실의 과열된 파이프 연결부',
    effect:'체온을 급격히 높여 극한의 추위에서 저체온증을 즉시 치료함.',
    note:'이끼가 숨을 쉴 때마다 칙- 소리와 함께 증기가 뿜어져 나온다. 겨울에 실험실 난로가 고장 났을 때 이 녀석들 세 뭉치를 이불 속에 넣고 잤는데, 아침에 일어나니 내 털이 찜질방 훈제 고양이처럼 꼬불꼬불해져 있었다. 뜨거운 걸 좋아하는 고양이가 있다면 최고의 친구겠지만, 나는 이제 증기 소리만 들으면 노이로제가 걸릴 것 같다.',
    gameEffect:'🌋 이그니스 제련 시간 단축 (겨울 시즌)',
  },
  {
    id:'resin', icon:'🌳', reqItem:'resin', reqAmt:8,
    name:'통곡의 덩굴', engName:'Wailing Vine',
    latin:'Lamentum creeper',
    habitat:'습도가 높고 그림자가 긴 폐허 뒷골목',
    effect:'해독제 성분이 매우 풍부하여 강력한 독성도 중화시킴.',
    note:'이 덩굴은 누군가 지나갈 때마다 바람 소리를 이용해 정말 구슬픈 비명소리를 낸다. 길드마스터가 처음 이걸 가져왔을 때, 나는 그가 죽은 줄 알고 울면서 30분 동안 장례식을 준비했다. 알고 보니 그냥 덩굴 소리였다. 그 이후로 덩굴을 볼 때마다 왠지 길드마스터가 떠올라서 밥을 안 먹게 된다. 다이어트 효과가 있긴 하다.',
    gameEffect:'🌿 숲의 정수 제조 시 수지 소모 -1',
  },
  {
    id:'rare', icon:'💎', reqItem:'rare', reqAmt:5,
    name:'고장 난 시계꽃', engName:'Clockwork Daisy',
    latin:'Horologium petal',
    habitat:'구식 기계 태엽 장치가 방치된 지하 터널',
    effect:'말린 꽃잎을 차로 마시면 일시적인 집중력 강화와 피로 회복에 탁월함.',
    note:'꽃잎이 초침처럼 딱딱거리는 소리를 내는데, 이게 주변 기계 장치의 진동과 공명하면 정말 이상한 일이 벌어진다. 어제 이걸 다루다가 꽃잎이 1분 일찍 피는 바람에, 내 기계 심장 수리 시간이 1분 앞당겨졌다. 세상이 1분 먼저 시작된 기분이다. 가끔은 시간을 거꾸로 돌리는 것 같은 착각이 들지만, 사실 그냥 내 시계가 늦게 가는 것 같다.',
    gameEffect:'⏳ 제련 완료 알림을 하루 전에 미리 표시',
  },
  // Bonus: Magnetic Bramble
  {
    id:'bramble', icon:'🧲', reqItem:'rare', reqAmt:10,
    name:'자석 가시나무', engName:'Magnetic Bramble',
    latin:'Spina ferrum',
    habitat:'고철 폐기장 근처, 금속 파편이 많은 곳',
    effect:'상처 부위에 가루를 뿌리면 금속 파편이나 이물질을 자석처럼 끌어당겨 제거함.',
    note:'길을 걷다 보면 자꾸 이 녀석들이 내 기계 장갑에 달라붙어서 떼어내느라 진땀을 뺀다. 처음엔 귀찮았는데, 실험 중에 실수로 잃어버린 작은 나사들을 찾을 때 이만큼 유용한 게 없다. 가끔 너무 강력해서 내 콧수염까지 달라붙을 때가 있는데, 그럴 때마다 내가 약초사인지 고철 수집가인지 헷갈린다. 분명히 말하지만, 금속 숟가락을 들고 이 근처에 가지 마라. 숟가락이 덩굴 속으로 빨려 들어가는 걸 보고 싶지 않다면.',
    gameEffect:'🔧 크리스탈 광산 채굴 시 금속 파편 추가 획득',
  },
  {
    id:'lubelily', icon:'🌼', reqItem:'lotus', reqAmt:12,
    name:'안갯속의 윤활유 꽃', engName:'Lubricant Lily',
    latin:'Olea micans',
    habitat:'기름 냄새 자욱한 지하 기계실 배수구',
    effect:'관절과 근육에 바르면 통증 완화 및 윤활 효과. 기계 다리 관절에도 효과 있음.',
    note:'꽃잎을 짓이기면 끈적하고 투명한 액체가 나오는데, 이게 정말이지 신기할 정도로 미끄럽다. 어제 실험실 바닥에 조금 흘렸다가 하루 종일 스케이트를 타는 꼴이 되었다. 그런데 신기한 건, 내 삐걱거리던 왼쪽 기계 다리 관절에 발랐더니 10년 묵은 녹이 씻은 듯이 사라졌다. 냄새가 딱 기계 기름 냄새라 다들 내가 꽃을 들고 있는 게 아니라 기름통을 들고 있는 줄 안다. 오해받기 딱 좋다.',
    gameEffect:'⚙️ 집 개조 비용 -10% / 워킹하우스 이벤트 확률 +5%',
  },
  {
    id:'rustcap', icon:'🍄', reqItem:'shroom', reqAmt:15,
    name:'녹슨 고철 버섯', engName:'Rust-Cap Spore',
    latin:'Fungus rubigo',
    habitat:'오래된 고철 더미, 습기 많은 철제 파이프 사이',
    effect:'상처에 뿌리면 딱딱하게 굳어 보호막 역할. 인공 갑옷 효과.',
    note:'이 녀석들은 자기가 진짜 고철인 줄 아는 모양이다. 갓 위에 붉은 녹이 가득 껴 있는데, 이걸 빻아서 상처에 바르면 순식간에 딱딱한 껍질이 형성된다. 편리하긴 한데, 마르고 나면 내 피부 색깔이 꼭 100년 된 수도꼭지 색깔처럼 변한다. 며칠 동안 씻어도 안 지워져서 사람들이 나를 보고 고양이 로봇이라고 부른다. 털은 복슬복슬한데 피부는 녹슬었다니, 정말 비극적인 조합이다.',
    gameEffect:'🛡️ 전투 피해 -15% / Vitality 최대치 +5',
  },
  {
    id:'voltcreep', icon:'⚡', reqItem:'rare', reqAmt:8,
    name:'감전되는 덩굴풀', engName:'Volt-Creeper',
    latin:'Fulmen vitis',
    habitat:'전력이 흐르는 배전반 근처, 번개 치는 고지대',
    effect:'미세 전류로 신경 자극. 무기력 해소 및 각성 효과(카페인 대용).',
    note:'절대, 정말 절대 맨손으로 꺾지 마라. 꺾는 순간 찌릿하는 느낌과 함께 털이 전부 곤두선다. 덕분에 아침에 졸릴 때 이 녀석 잎사귀를 살짝 만지면 커피 열 잔보다 강력한 각성 효과가 있다. 다만, 너무 많이 만지면 하루 종일 내 몸에서 정전기가 발생해서, 고철 나사들이 내 주변으로 20cm 정도 둥둥 떠다니는 장관을 볼 수 있다. 밤에 길 잃을 일은 없겠군. 내 몸이 야광처럼 번쩍거리니까.',
    gameEffect:'⚡ Activity 최대치 +1 (영구) / 정밀 탐색 효율 +20%',
  }
];
...
// ══════════════════════════════════════
//  CUSTOMER SYSTEM (단골 손님 NPC)
// ══════════════════════════════════════
const CUSTOMERS = [
  {
    id:'fisher', avatar:'🐟', name:'늙은 어부 고양이',
    title:'항구 마을에서 온 상인',
    says:[
      '"관절이 요즘 영 시원찮소... 좋은 약 있소?"',
      '"바다 냄새가 배어도 이 약만큼은 믿소."',
      '"파도가 거칠수록 약이 더 필요하지 않겠소."',
    ],
    orders:[
      {item:'healing', name:'치유 물약', count:2, reward:{bronze:15}, deadline:2},
      {item:'moon',    name:'달빛 물약', count:1, reward:{bronze:12}, deadline:3},
    ],
    specialReward:{herb:{resin:3}, msg:'고마소. 바다에서 주운 수지라오.'},
  },
  {
    id:'guild', avatar:'⚔️', name:'길드마스터',
    title:'모험가 길드 대표',
    says:[
      '"급하게 해독제가 필요해. 조건은 맞춰주겠어."',
      '"전쟁터에서 이 약이 몇 명을 살렸는지 알아?"',
      '"통곡의 덩굴 덕분에 살아남은 길드원이 열 명이야."',
    ],
    orders:[
      {item:'forest',  name:'숲의 정수', count:1, reward:{silver:1,bronze:5}, deadline:2},
      {item:'dream',   name:'꿈의 물약', count:2, reward:{silver:1},          deadline:3},
    ],
    specialReward:{herb:{moss:4,shroom:3}, msg:'"길드 창고에서 꺼냈어. 쓸모있을 거야."'},
  },
  {
    id:'noble', avatar:'👑', name:'귀족 고양이 부인',
    title:'Aethel Cataria 상류층',
    says:[
      '"제 딸아이 피부가 너무 창백해서요. 도움이 될까요?"',
      '"이런 곳까지 직접 오다니... 하지만 효과가 있더군요."',
      '"소문으로 들었어요. Baba의 물약이 최고라고요."',
    ],
    orders:[
      {item:'moon',    name:'달빛 물약', count:1, reward:{silver:1,bronze:8}, deadline:3},
      {item:'legendary',name:'전설의 영약',count:1,reward:{golden:1},         deadline:5},
    ],
    specialReward:{bronze:20, msg:'"감사해요. 사례는 충분히 드리겠어요."'},
  },
  {
    id:'traveler', avatar:'🎒', name:'떠돌이 약초사',
    title:'먼 지방에서 온 동료',
    says:[
      '"선배 약초사님 맞죠? 배울 게 있어서 왔어요."',
      '"저도 언젠가 이런 정원을 만들고 싶어요."',
      '"희귀초는 어디서 구하세요? 저는 통 못 찾겠더라고요."',
    ],
    orders:[
      {item:'healing', name:'치유 물약', count:3, reward:{bronze:18,herb:{herb:5}}, deadline:3},
      {item:'forest',  name:'숲의 정수', count:1, reward:{bronze:15,herb:{rare:1}}, deadline:2},
    ],
    specialReward:{herb:{lotus:3,herb:5}, msg:'"제가 모은 재료예요. 받아주세요!"'},
  },
  {
    id:'mysterious', avatar:'🌑', name:'수상한 고양이',
    title:'정체불명',
    says:[
      '"...약 하나만. 이름 묻지 마."',
      '"값은 충분히 치르겠어. 조용히 거래하자."',
      '"이 정원... 전보다 커진 것 같군."',
    ],
    orders:[
      {item:'legendary',name:'전설의 영약',count:1,reward:{golden:2},         deadline:4},
      {item:'dream',    name:'꿈의 물약', count:2, reward:{silver:2},          deadline:2},
    ],
    specialReward:{golden:1, msg:'"...고마워. 언젠가 도움이 될 거야."'},
  },
];
