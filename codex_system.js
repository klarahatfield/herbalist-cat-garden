...
function renderResearchNotes() {
  ...
  RESEARCH_NOTES.forEach(function(note) {
    ...
    var card = document.createElement('div');
    card.style.cssText = 'border-radius:10px;border:1.5px solid ' + (isUnlocked ? 'rgba(184,137,26,.4)' : 'rgba(150,130,80,.15)') + ';background:' + (isUnlocked ? 'rgba(60,45,15,.4)' : 'rgba(30,25,10,.2)') + ';margin-bottom:8px;overflow:hidden';
    ...
    if(!isUnlocked) {
      ...
      card.innerHTML = '<div style="display:flex;align-items:center;gap:10px;padding:10px 13px;cursor:pointer;border-bottom:1px solid rgba(184,137,26,.3)" onclick="toggleNoteCard(\'' + bodyId + '\',\'' + note.id + '\')">'
        + '<span style="font-size:1.5rem">' + note.icon + '</span>'
